...
const Piece: NextPage = () => { // 🪝
  ...
  colors.data ?. map((color) => {
    color_choices.push({value: color.id, label: color.name})
  });
  ...
  return (
    <>
      ...
      <div>
        <select onChange={
            event => color_change_handler(event.target.value)
          }
        ...
        {
        nascent_piece.colorId !== 'default' && <div>
          <GridBoard board_color='grey'
            piece={nascent_piece}
            square_size={40}
            square_click_handler={square_click_handler}/>
          <button onClick={addPiece}
            className='btn btn-primary'>Add piece</button>
        </div>
      } </div>
